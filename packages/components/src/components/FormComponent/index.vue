 ...
 * @LastEditTime: 2023-08-28 17:04:58
    ...
    <el-row>
      <el-col v-for="item in formConfig" :key="item.id" :span="item.col || 8">
        <el-form-item :label="`${item.label}`" :label-width="(item.labelWidth || 70) + 'px'">
          <component v-bind:is="components[item.type]" @changeValue="changeValue"
          :searchProps="item"></component>
        </el-form-item>
      </el-col>
...
const changeValue = (col:string,val:string)=>{
  searchParam.value[col] = val;
}
